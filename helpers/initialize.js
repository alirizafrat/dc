...
class Initialize {
    ...
    hello(client) {
        ...
        readdir(__dirname.replace('HELPERS', 'SERVER') + `/${this.client.name}/app/`).then((appFolders) => {
            appFolders.forEach(async (intType) => {
                readdir(__dirname.replace('HELPERS', 'SERVER') + `/${this.client.name}/app//${intType}/`).then((raw_output) => {
    ...
    async loader() {
        ...
        const elements = await readdir(__dirname + `/../SERVER/${this.client.name}/Events/`);
        ...
        elements.forEach(async (element) => {
            if (element.endsWith(".js")) {
                ...
                const event = new (require(__dirname + `/../SERVER/${this.client.name}/Events/${element}`))(this.client);
                this.client.on(element.split(".")[0], (...args) => event.run(...args));
                delete require.cache[require.resolve(__dirname + `/../SERVER/${this.client.name}/Events/${element}`)];
            } else {
                const detaileds = await readdir(__dirname + `/../SERVER/${this.client.name}/Events/${element}/`);
                ...
                detaileds.forEach((detail) => {
                    ...
                    const event = new (require(__dirname + `/../SERVER/${this.client.name}/Events/${element}/${detail}`))(this.client);
                    this.client.on(element.split(".")[0], (...args) => event.run(...args));
                    delete require.cache[require.resolve(__dirname + `/../SERVER/${this.client.name}/Events/${element}/${detail}`)];
                });
            }
        });
    }
