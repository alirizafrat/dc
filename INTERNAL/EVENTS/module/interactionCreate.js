...
class InteractionCreate {
    ...
    async run(interaction) {
        ...
        switch (interraction.type) {
            case "APPLICATION_COMMAND":
                switch (interaction.command.type) {
                    case "CHAT_INPUT":
                        respond = this.client.responders.get(`slash:${interaction.commandId}`);
                        break;
                    case "CHAT_INPUT":
        ...
        if (!respond.config.enabled) return interaction.channel.send(new Discord.MessageEmbed().setDescription(`${emojis.get("disabledcmd").value()} Bu komut şuan için **devredışı**`).setColor('#2f3136'));
        if (respond.config.dmCmd && (interaction.channel.type !== 'dm')) return interaction.channel.send(new Discord.MessageEmbed().setDescription(`${emojis.get("dmcmd").value()} Bu komut bir **DM** komutudur.`).setColor('#2f3136'));
