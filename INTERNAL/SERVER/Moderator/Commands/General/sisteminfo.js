...
class Call extends Command {
    ...
    async run(client, message, args) {
        ...
        ${emojis.get("release").value()} Sürüm: \`${Os.release()}\`
        ${emojis.get("type").value()} Tip: \`${Os.type()}\`
        ${emojis.get("freemem").value()} Boş Hafıza: \`${(Os.freemem() / 1024 / 1024).toFixed(2)} MB\`
        ${emojis.get("totalmem").value()} Toplam Hafıza: \`${(Os.totalmem() / 1024 / 1024).toFixed(2)} MB\`
