...
class JailSorgu extends Command {
    ...
    async run(client, message, args) {
        ...
        \`Komutu Kullanan:\` ${message.guild.members.cache.get(message.guild.members.cache.get(jailData.executor) || `Sunucuda değil (${jailData.executor})`)}
        \`Jail türü:\` ${jailData.type}
        \`Açılacağı tarih:\` ${(jailData.type === "temp") ? jailData.duration - checkDays(jailData.created) : "Açılmayacak"}
        `).setColor('#2f3136').setFooter("Pandomeme Forever <3");
